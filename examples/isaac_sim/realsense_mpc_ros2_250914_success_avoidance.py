...
class CuroboMpcNode(Node):
    ...
    def depth_callback(self, msg):
        ...
        # ================== 추가 시작 ==================
        # 1.0 미터(1000mm)를 초과하는 모든 깊이 값을 0으로 설정합니다.
        # RealSense는 mm 단위로 값을 보내므로, 1000.0으로 비교합니다.
        # 만약 ROS 드라이버에서 미터 단위로 변환했다면 1.0으로 비교해야 합니다.
        # 우선 1000.0으로 시도해보시고, 안되면 1.0으로 바꿔보세요.
        self.depth_image[self.depth_image > 1000.0] = 0.0
        # ================== 추가 끝 ====================
    ...
    def realsense_callback(self):
        ...
        # self.world_model.decay_layer("world")  # 테스트 중엔 끄는 걸 권장
        ...
        try:
            H, W = Z.shape
            stride = 2  # 다운샘플(시각화만 하므로 2~4 권장)
            yy, xx = np.mgrid[0:H:stride, 0:W:stride]
            z = Z[0:H:stride, 0:W:stride]
        ...
        if self.args.show_window and self.color_image is not None:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                ...
                rclpy.shutdown()
        duration = (time.time() - start_time) * 1000 # ms 단위로 변환
